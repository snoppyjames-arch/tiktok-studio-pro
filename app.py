...
async function generate(){
  ...
  document.getElementById('result').innerHTML = `
    ...
    <img class="ai-img" src="${data.image1}" loading="lazy"><br><small>AI Image 1</small></div>
    <div class="card"><h3>2. HOOK</h3><p>${data.hook}</p></div>
    <div class="card"><h3>3. FULL SCRIPT</h3><p>${data.script.replace(/\\n/g,'<br>')}</p>
    <img class="ai-img" src="${data.image2}" loading="lazy"><br><small>AI Image 2</small></div>
    <div class="card"><h3>4. HASHTAGS</h3><p>${data.hashtags}</p></div>
    <div class="card"><h3>5. BEST TIME</h3><p>${data.best_time}</p>
    <img class="ai-img" src="${data.image3}" loading="lazy"><br><small>AI Image 3 - Thumbnail</small></div>
    <div class="card"><h3>6. CAPTION</h3><p>${data.caption}</p>
    <button class="btn" onclick="navigator.clipboard.writeText(\`${data.caption}\`);alert('Copied!')">Copy Caption</button>
    </div>
  `;
...
def generate():
    ...
    hashtags = f"#{idea.replace(' ','')} #viral #tiktokcameroon #yaounde #fyp #foryou"
    best_time = "Today 7:00 PM - 9:30 PM Yaounde time (WAT)"
