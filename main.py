...
def gen_pic(m):
    ...
    if not prompt:
        bot.reply_to(m, "❌ Type: /pic a luxury villa Dubai sunset 4k"); return
...
def gen_video(m):
    ...
    if not raw:
        bot.reply_to(m, "❌ Type: /video a lion roaring mountain 40s"); return
duration = 40
...
try:
    ...
    for i in range(duration // 5):
        ...
        open(p,'wb').write(img_data)
        paths.append(p)
        time.sleep(1)

    out = f"/tmp/final_{m.chat.id}.mp4"
    clip = ImageSequenceClip(paths, fps=1/5).set_duration(duration)
    ...
    bot.send_video(m.chat.id, open(out,'rb'), caption=caption, supports_streaming=True)

    for p in paths: os.remove(p)
    os.remove(out)

except Exception as e:
    # If moviepy fails, send as photos
    bot.reply_to(m, f"⚠️ Video merge failed (moviepy). Sending as HD photo sequence - you can make video in CapCut in 1 click:\nError: {e}")
